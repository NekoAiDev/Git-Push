...
2.2 我们收集的匿名统计数据仅限于以下字段，绝不包含任何可识别您个人身份的信息，也绝不包含您的文件内容、仓库地址、文件路径、计算机硬件信息或任何系统隐私数据：
    ...
    （八）您的公网 IP 地址及其所属地区（省/市级）。该信息由服务器在接收上报请求时自动获取，仅用于后台管理员查看设备的大致地理分布，不进行精确地理位置追踪，亦不用于识别您个人身份或进行用户画像。您亦可在「设置 → 隐私与统计」中手动选择所在省份，后台将优先显示您手动设置的地区（IP 库对国内 IPv6 定位常有偏差，手动设置最准确）。
...
1.8 公网 IP 地址与所属地区（省/市）：由服务器在接收上报请求时，从网络请求头中自动获取您的公网 IP，并结合第三方 IP 地理库（ipinfo.io）解析出所属的省/市级地区。该信息仅用于后台管理员查看在线设备的地理分布，不进行精确追踪，亦不用于用户画像。您亦可在「设置 → 隐私与统计」中手动选择所在省份，后台将优先显示您手动设置的地区（IP 库对国内 IPv6 定位常有偏差，手动设置最准确）。
...
4.5 您的计算机隐私信息：本软件不会收集您的硬件序列号、MAC 地址、操作系统详细配置、已安装软件列表、浏览记录等与代码推送无关的计算机信息；本软件亦不会收集您的精确地理位置（如 GPS 坐标）。关于公网 IP 与所属省/市地区的收集，已在本文第一节第 1.8 条明确说明，其仅用于后台设备地理分布展示，不进行精确追踪。
...
class GitPushTool:
    ...
    def _open_settings(self):
        ...
        ttk.Label(frm_privacy, text="IP 库对国内 IPv6 定位常有偏差，手动选择省份最准确；选“自动”则按 IP 库估算", style="Small.TLabel").pack(anchor="w")
